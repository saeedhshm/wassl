  ...
  final Order? order;
   NewLoanOrder({Key? key,this.onClose, this.order}) : super(key: key){
     if(order != null) {
       ...
       controller.loanStartingDate = DateTime(int.tryParse(endDateArr?[0] ?? '') ?? 0,int.tryParse(endDateArr?[1] ?? '') ?? 0,int.tryParse(endDateArr?[2] ?? '') ?? 0,);
        ...
        children: [
          Column(
            children: [
              ...
              Obx(() => Expanded(
                  child: controller.loadingLoansTypes.value ? const Center(
