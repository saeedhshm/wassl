...
class ApologyRequestController extends GetxController {
  ...
  Future updateRequest(String orderId) async {
    ...
    loading.value = true;
    var response = await AppApiHandler.postDataWithFile(
        url: '${AppUrls.updateTabreerApi}/$orderId',
        body: body,
        header: appController.appHeader,
        fileName: filePath);
  ...
  Future cancelRequest(String orderId) async {
    var response = await AppApiHandler.putData(
      url: '${AppUrls.cancelTabreerApi}/$orderId',
      header: appController.appHeader,
    );
  ...
  getLetterTypes() async {
    ...
    orderTypes.value = await TypesController()
            .getTypes(AppUrls.tabreerTypesApi, appController.appHeader) ??
        OrderTypesRetriever();
  ...
  setDate(String date) {
    ...
    this.date = DateTime(
      int.tryParse(splitDate[0]) ?? 0,
      int.tryParse(splitDate[1]) ?? 0,
      int.tryParse(splitDate[2]) ?? 0,
    );
  }
