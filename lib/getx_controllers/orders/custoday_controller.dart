...
class CustodyRequestController extends GetxController {
  ...
  Future addNewPermission() async {
    ...
    var body = {'type': '${selectedType?.id}', 'reason': '$reason'};

    loading.value = true;
    var response = await AppApiHandler.postDataWithFile(
        url: AppUrls.addCustody,
        body: body,
  ...
  Future updateRequest(String orderId) async {
    ...
    loading.value = true;
    var response = await AppApiHandler.postDataWithFile(
        url: '${AppUrls.updateCustody}/$orderId',
        body: body,
        header: appController.appHeader,
        fileName: filePath);
  ...
  Future cancelRequest(String orderId) async {
    var response = await AppApiHandler.putData(
      url: '${AppUrls.cancelCustody}/$orderId',
      header: appController.appHeader,
    );
  ...
  getTypes() async {
    ...
    orderTypes.value = await TypesController()
            .getTypes(AppUrls.getCustodyTypes, appController.appHeader) ??
        OrderTypesRetriever();
