...
class OrderDetailsController extends GetxController {
  ...
  Future<bool> setTeamOrderStatusRequest(
      String orderStatus, String comment) async {
    var body = {
    ...
    final response = await AppApiHandler.postData(
        url: AppUrls.setTeamOrderSatus, body: body, header: headers);

    if (response.statusCode == 200) {
      final message = orderStatus == '2'
          ? 'order_approved_success'.tr
          : 'order_disapproved_success'.tr;
  ...
  bool get isTeamOrder {
    // return true;
    return (appController.loginModel.value.user?.id ?? 0) ==
        (order.activeResponsibleId ?? 0);
  }
  ...
  Future<Uint8List> downloadPdfFile(String pdfUrl) async {
    var response = await AppApiHandler.getData(
        url: pdfUrl, header: appController.appHeader);
    var listOfBytes = response.bodyBytes;
    var filePath = await FileUtility.writeFileToDirectory(
        listOfBytes, pdfUrl.split('/').last);
