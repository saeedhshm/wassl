...
class AskPermissionController extends GetxController {
  ...
  Future addNewPermission() async {
    ...
    var body = {
      ...
      'date':
          '${permissionDate?.year}-${permissionDate?.month}-${permissionDate?.day}',
  ...
  Future updateRequest(String orderId) async {
    ...
    var response = await AppApiHandler.postDataWithFile(
        url: '${AppUrls.updatePermission}/$orderId',
        body: body,
        header: appController.appHeader,
        fileName: filePath);
  ...
  Future cancelRequest(String orderId) async {
    var response = await AppApiHandler.putData(
      url: '${AppUrls.cancelPermission}/$orderId',
      header: appController.appHeader,
    );
  ...
  getPermissionsTypes() async {
    ...
    orderTypes.value = await TypesController()
            .getTypes(AppUrls.getPermissionsTypes, appController.appHeader) ??
        OrderTypesRetriever();
  ...
  var reasonTypes = [
    OrderType(
      id: 1,
    )..name = 'personal_permission'.tr,
    OrderType(
      id: 2,
    )..name = 'illness_permission'.tr,
    OrderType(
      id: 3,
    )..name = 'emergency_permission'.tr,
  ];
  ...
  set timeIn(selectedTime) {
    if (selectedTime != null) {
      ...
      var timeIn24 = dateFormatOf24Hours
